...
function signUpUser() {
	...
	var email = helper.getData('userEmail'); 
	if (email.length === 0){
	...
	var password = helper.getData('userPassword'); 
	...
	var phoneNum = helper.getData('userTel'); 
	...
	firebase.auth().createUserWithEmailAndPassword(email, password).then(function(result){
		var createProfile = _User.addNew({
	    	...
	    	email: email,
	    	firstname: helper.getData('fName'), 
	    	lastname: helper.getData('lName'), 
	    	password: password,
	    	phone: phoneNum,
	    	profile_picture : '', 
